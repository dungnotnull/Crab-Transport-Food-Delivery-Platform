...
## 1. Mục tiêu dự án
Crab Transport Platform - Hệ thống đặt xe thời gian thực.
...
- **Naming Convention**: 
  ...
  - `kebab-case` cho tên file/folder (VD: `trip.controller.ts`).
...
- Sử dụng **Event Emitter** (`@nestjs/event-emitter`) cho các tác vụ bất đồng bộ (Async) nhằm giảm coupling (Ví dụ: Trip created -> Emit event -> Tracking module bắt event khởi tạo Socket Room).
