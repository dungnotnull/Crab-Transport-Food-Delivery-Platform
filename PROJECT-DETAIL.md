...
1. **Ràng buộc Nhận cuốc (Concurrency / Race Condition)**:
   - Một đơn hàng chỉ có thể được nhận bởi MỘT tài xế duy nhất. Khi thuật toán Broadcast phát cuốc cho top 5 tài xế, nếu nhiều người cùng bấm nhận 1 lúc, Backend phải dùng **Pessimistic Locking** (`SELECT FOR UPDATE`) để khóa dòng dữ liệu của Order đó lại. Ai đến trước được trước, người đến sau nhận lỗi `409 Conflict`.
2. **Ràng buộc Tài xế (Driver Constraints)**:
   - **Trạng thái**: Tài xế bắt buộc phải `is_online = true` mới lọt vào thuật toán Matching.
   - **Số lượng cuốc**: Tài xế không thể nhận thêm đơn mới nếu trường `active_order_id` khác NULL (đang bận 1 cuốc xe khác).
3. **Quy tắc Hủy chuyến (Cancellation Rules)**:
   - **Khách hàng**: Được hủy khi đơn ở trạng thái `FINDING_DRIVER` hoặc `ACCEPTED`. Bị CẤM hủy khi đã tới bước `IN_TRANSIT` hoặc `WAITING_FOR_FOOD`.
   - **Tài xế**: Nếu tài xế hủy (do xe hỏng, ko liên lạc được khách), `active_order_id` của tài xế về NULL, và Order sẽ được tự động đổi lại thành `FINDING_DRIVER` để hệ thống tìm tài xế thay thế.
