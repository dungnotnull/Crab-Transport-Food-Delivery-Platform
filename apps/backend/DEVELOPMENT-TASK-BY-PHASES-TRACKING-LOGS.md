...
- [x] **Task 2.3: Trip Module - Entity & Enums (`modules/trips`)**
  - [x] Tạo bảng `Trip`: `id`, `customer_id`, `driver_id`, `pickup_location`, `dropoff_location`, `status`, `total_fare`.
  - [x] Enum `TripStatus`: `FINDING_DRIVER`, `ACCEPTED`, `ARRIVED_AT_PICKUP`, `IN_TRANSIT`, `ARRIVED_AT_DESTINATION`, `COMPLETED`, `CANCELLED`.
- [x] **Task 2.4: Trip Module - State Machine Logic**
  - [x] Xây dựng State Transition Guard: Hàm check logic cấm chuyển trạng thái nhảy cóc.
- [x] **Task 2.5: Trip API (Booking) & Geospatial Constraint**
  - [x] API POST `/api/v1/trips/book`: Nhận toạ độ -> Gọi Routing -> Gọi Pricing.
  - [x] **Ràng buộc địa lý**: Thêm logic báo lỗi `400 Bad Request` nếu `distance > 50km` (50,000m).
  - [x] Lưu DB trạng thái `FINDING_DRIVER`. Emit sự kiện `trip.created`.
- [x] **Task 2.6: Timeout/Auto-Cancel Cronjob (`modules/cron`)**
  - [x] Viết `CronJob` (chạy mỗi 1 phút): Quét các Trip có `status = FINDING_DRIVER` và `created_at` quá 5 phút. Tự động đổi sang `CANCELLED`.
- [x] **Task 2.7: State Machine Transition APIs**
  - [x] API `PATCH /api/v1/trips/:id/status` để Tài xế cập nhật trạng thái đơn hàng.
...
- [x] **Task 3.1: Driver Location Management (`modules/drivers`)**
  ...
  - [x] Cột `current_location` (Kiểu `Point`, srid `4326`, index `GiST`). Cột `is_online`, `active_trip_id`.
...
- [x] **Task 3.2: Matching Algorithm (Thuật toán)**
  ...
  - [x] **Ràng buộc tài xế**: Bắt buộc WHERE `is_online = true` VÀ `active_trip_id IS NULL`. Khoảng cách <= 3000m. LIMIT 5.
- [x] **Task 3.3: Dispatch Flow (Event Listener)**
  - [x] Bắt sự kiện `trip.created`, gọi hàm Matching tìm top 5.
  - [x] Phát socket `driver:trip_offer` tới 5 tài xế, đính kèm `expiredAt` (15s TTL).
- [x] **Task 3.4: Accept Trip API (Pessimistic Locking)**
  - [x] API POST `/api/v1/trips/:tripId/accept`.
  ...
  - [x] Cập nhật Trip sang `ACCEPTED`, cập nhật `DriverLocation.active_trip_id`.
- [x] **Task 3.5: Cancellation Flow (Quy tắc hủy)**
  - [x] API POST `/api/v1/trips/:tripId/cancel`. Lấy role từ JWT. 
  - [x] **Customer**: Chặn lỗi `400` nếu status là `IN_TRANSIT`. Nếu hủy hợp lệ, đổi Trip sang `CANCELLED` và giải phóng Driver (`active_trip_id = NULL`).
  - [x] **Driver**: Được phép hủy. Giải phóng Driver (`active_trip_id = NULL`). Bắt buộc reset Trip quay lại `FINDING_DRIVER` và emit lại luồng Matching tìm người khác.
...
- [x] **Task 4.1: Socket.io Gateway Setup (`modules/tracking`)**
  ...
  - [x] Logic Join Room: `client.join("trip_" + tripId)`.
- [x] **Task 4.2: Telemetry Pinging & Broadcasting**
  ...
  - [x] Emit `trip:location_stream` (Customer xem live marker).
- [x] **Task 4.3: Driver Simulator Script (`modules/simulator`)**
  - [x] Cài đặt `@turf/turf`. API POST `/api/v1/simulator/simulate-trip`.
  - [x] Dùng `setInterval`, `turf.length`, `turf.along` chạy mô phỏng tick 1.5s. Giả lập pause 5s tại điểm đón rước khách.
...
- [x] **Task 6.1: Review Entity & APIs**
  ...
  - [x] API `POST /api/v1/trips/:id/rating` để Customer gửi đánh giá khi cuốc xe kết thúc.
...
- **Tính năng**:
  ...
  - `[x]` Update Trip flow: Áp dụng Coupon (Preview & Book), Pessimistic Locking chống Race-condition.
  - `[x]` Update Trip Complete: Cập nhật Ví tài xế (Cộng/Trừ phí hoa hồng).
...
- **Tính năng**:
  - `[x]` Entity `Trip`: Thêm `PaymentMethod` (CASH, CREDIT_CARD, E_WALLET) và `PaymentStatus`.
...
## Phase 9: Driver Monitoring & Priority Matching - [COMPLETED]
- **Tính năng**:
  - `[x]` Entity `Trip`: Thêm `estimated_duration` lưu thời gian cuốc xe dự kiến.
